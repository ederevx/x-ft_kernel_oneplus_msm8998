					...
					bool set_wake)
{
	...
	if (!set_wake) {
		irqs = uirqs->enable_irqs;
		lpm_vote = &uirqs->enable_vote;
	} else {
		irqs = uirqs->wake_irqs;
		lpm_vote = &uirqs->wake_vote;
	}
	...
	lpm_wake = *lpm_vote == bitmap_empty(irqs, 
			uirqs->domain->revmap_size);
	if (lpm_wake)
		*lpm_vote = !*lpm_vote;
